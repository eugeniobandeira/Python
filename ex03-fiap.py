texto = 'exercício 3, cap 04 - quando a máquina começa a tomar decisões'
print('*' * len(texto))
print(texto.upper())
print('*' * len(texto))
soma_nota_impar = 0
...
for aluno in range(1, total_alunos_turma + 1):
    if aluno % 2 == 1:
        aluno_impar_nota = float(input(f'Você está digitando as notas dos alunos ímpares, por favor, informe a nota aluno nº {aluno}: '))
...
for aluno in range(1, total_alunos_turma + 1):
    if aluno % 2 == 0:
        aluno_par_nota = float(input(f'Você está digitando as notas dos alunos pares, por favor, informe a nota aluno nº {aluno}: '))
